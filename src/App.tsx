...
function App() {
  ...
  const [pokesPerPage, setPokesPerPage] = useState(30);
  ...
  const loadMore = () => {
    if (searchPoke.length > 0) setPokesPerPage(pokesPerPage + 30);
    else setPokesPerPage(pokesPerPage + 30);
  };
  ...
  return (
    <StyledContainer
      fluid
      style={{
        backgroundColor: `${darkTheme ? "#103454" : "#2575bc"}`,
    ...
    >
      <StyledGrid>
        ...
        <StyledColumn xs={12} sm={1} md={1} lg={1} xl={1}>
          <Button
            ...
            href="https://github.com/AlexElguetaDev"
        ...
        <StyledColumn>
          <ActionIcon color="red" size="xl" radius="xl" variant="filled" style={{
            width: '150px',
            height: '100px',
            fontSize: '20px'
          }}
            onClick={() => { setShowContent(true); scrollTo({ y: 900 }) }}
          >
            START
          </ActionIcon>
        </StyledColumn>
      ...
      {showContent && (
        <StyledContainer
          fluid
          style={{
            backgroundColor: `${darkTheme ? "#6d1a0c" : "#ca2f13"}`,
            transition: "all 0.5s linear",
          }}>
          {scrollY > 1200 && (
            <>
...
const StyleGridPokemon = styled(Grid)`
  display: grid;
